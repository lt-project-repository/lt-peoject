...
def create_model(use_selfatt=False, use_fc=False, dropout=None, stage1_weights=False, dataset=None, log_dir=None,
                 test=False, *args):
    ...
    if not test:
        if stage1_weights:
            ...
            if log_dir is not None:
                weight_dir = path.join('/'.join(log_dir.split('/')[:-1]), 'resnet10_uniform_e90')
            else:
                weight_dir = './logs/%s/models/resnet10_uniform_e90' % dataset
