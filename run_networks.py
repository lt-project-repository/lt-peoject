...
class model():
    ...
    def __init__(self, config, data, test=False, meta_sample=False, learner=None):
        ...
        # Initialize model
        self.init_models()
        total = sum([param.nelement() for param in self.networks['feat_model'].parameters()])
        print(total)
        # pdb.set_trace()
    ...
    def callback_after_epoch(self, epoch):
        ...
        if True:
            ...
            # np.savez('./numpy/' + str(epoch) + 'neg_gad.npy', self.accum_neg_logit_grad_epoch.cpu().numpy())
    ...
    def train(self):
        ...
        # without logit normalization
        self.training_opt['norm_logits'] = False
        self.eval('test' if 'test' in self.data else 'val')

        # with logit normalization
        self.training_opt['norm_logits'] = True
        self.eval('test' if 'test' in self.data else 'val')
        print('Done')
    ...
    def eval(self, phase='val', openset=False, save_feat=False):
        ...
        print_write(print_str, self.log_file)
        print_str = ['logits normalization: %s' % (self.training_opt['norm_logits'])]
        print_write(print_str, self.log_file)
        time.sleep(0.25)
